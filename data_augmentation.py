...
parser.add_argument('--nAug', type=int, default=10000, help='number of augmentation')
...
def create_random_input(num):
    random_data = np.random.uniform(low=0,high=1,size=[num, seq_length, i_dim])
    return random_data
...
def main():
    ...
    with open('{0}/model_gan.json'.format(loadpath, ),'r') as f:
        ...
        model.load_weights('{0}/gan_param_epoch{1}.hdf5'.format(loadpath, epoch))
    ...
    # for i, j in enumerate(X[:4]):
    #     plt.subplot(2, 2, i+1)
    #     plt.plot(j)
    # plt.plot(X[:10].T)
    # plt.show()
    K.clear_session()
    np.save('{0}/dataset/ecg_{1}_based_{2}.npy'.format(filedir,TYPEFLAG, DATATYPE), X)
