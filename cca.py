...
Loadpath = '{0}/{1}/{2}/class{3}'.format(filedir, dirs, datadir, Class, l_ind)
filepath = '{0}/{1}/{2}/class{3}/load{4}'.format(filedir, dirs, datadir, Class, l_ind)
...
def trans_data(src, z):
    ...
    #     print('error')
    peak_to_peak = abs(downpeak_ind - uppeak_ind)
...
def disp_subplot(src, n=[1,1], name=None, axis_fix=False, ylim=[None, None],plot_type='line'):
    ...
    for i, j in enumerate(src):
        ...
        if plot_type == 'line':
            ...
            # ax.bar(range(len(j)), j, tick_label=label, color='w', edgecolor='k')
            ax.bar(np.arange(len(j)), j, bar_width, color='w', edgecolor=e)
            ...
            # for x, y in zip(range(len(j)), j):
                # plt.text(x, y*1.1, '{:.2f}'.format(y), ha='center', va='top')
        if axis_fix:
            ax.set_ylim(ymin, ymax)
    plt.tight_layout()
    # plt.tick_params(labelsize=30)
    fig.savefig('{0}/{1}.eps'.format(filepath, name))
    plt.close()
    if plot_type == 'bar':
        with open('{0}/{1}.csv'.format(filepath, name), 'w') as f:
...
def calc_cca(x1, x2, low_corr=0):
    ...
    with open('{0}/corr_class{1}.csv'.format(Loadpath, Class), 'w') as f:
        writer = csv.writer(f, lineterminator='\n')
        writer.writerow(corr_list)
...
def control_z(src, c):
    loadpath = '{0}/lstm-gan/backup1007/{1}_split_No0'.format(filedir, datadir)
...
def main():
    ...
    for cc, i in enumerate(np.unique(x[..., -1])):
        ...
        plt.ylim([0,1])
        plt.title('{}'.format(cc))
        plt.savefig('{0}/CCA/{2}/train_average_class{1}.png'.format(filedir, cc, datadir))
        plt.close()
        plt.figure(figsize = (16, 9))
        plt.bar(range(len(trans_x1.mean(axis=0))), trans_x1.mean(axis=0), yerr = trans_x1.std(axis=0))
        plt.title('{}'.format(cc))
        # plt.xticklabels(label)
        plt.savefig('{0}/CCA/{2}/train_feature_hist_class{1}.png'.format(filedir, cc, datadir))
        plt.close()
        src = src.mean(axis=0)
        buff.append(src)
    ...
    x = np.load('{0}/class{1}.npy'.format(Loadpath, Class))
    x1 = x[..., 0]
    x2 = x[..., 1]
    ...
    x = np.load('{0}/walk2_class{1}.npy'.format(Loadpath, Class, args.pos))
    x1 = x[..., 0]
    x2 = x[..., 1]
    trans_x1, trans_x2 = trans_data(x1, x2)
