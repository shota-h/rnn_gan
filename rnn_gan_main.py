...
def create_random_input(signal_num):
    random_data = np.random.uniform(low=0,high=1,size=[signal_num,1,i_dim])
...
def form_discriminator():
    ...
    D.add(LSTM(units=1,unit_forget_bias=True,return_sequences=True,recurrent_regularizer=l2(0.01)))
...
def form_generator():
    ...
    for i in range(layer_num-1):
        ...
        # G.add(BatchNormalization(momentum=0.9,beta_initializer=initializers.constant(value=0.5),gamma_initializer=initializers.constant(value=0.1)))
    G.add(LSTM(units=1,unit_forget_bias=True,return_sequences=True,recurrent_regularizer=l2(0.01)))
...
def main():
    ...
    np.save('{0}/loss_d_mat.npy'.format(file_path),np.array(loss_d_mat))
    np.save('{0}/loss_g_mat.npy'.format(file_path),np.array(loss_g_mat))
    np.save('{0}/predict_d.npy'.format(file_path),np.array(predict_d_mat))
    np.save('{0}/predict_g.npy'.format(file_path),np.array(predict_g_mat))
