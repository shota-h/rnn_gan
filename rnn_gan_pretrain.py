...
if __name__=='__main__':
    train_x=np.load(file_dir+'/ecg_only_one.npy')
    global signal_len
    signal_len=train_x.shape[1]
    v_size=10
    varidation_x=train_x[:v_size]
    train_x=train_x[v_size:]
    ...
    varidation_y=np.zeros([v_size,1,1])
    varidation_y=np.append(varidation_y,np.ones([v_size,1,1]),axis=0)
    mat_d=[]
    mat_g=[]



    for epoch, batch_num in itertools.product(range(100),range(2)):
        if batch_num==0:
