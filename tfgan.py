...
from keras.layers import Input, Dense, Activation, pooling, Reshape, Masking, Lambda, RepeatVector, Add
...
class create_valila_model():
    def __init__(self, writer):
        ...
        self.dis = self.build_time_discriminator()
    ...
    def build_generator(self):
        ...
        input = Input(shape = (None, latent_vector))
    ...
    def build_freq_discriminator(self):
        ...
        # with tf.device('/cpu:0'):
        input = Input(shape = (None, feature_count))
        fft_layer_r = Lambda(lambda x: K.dot(x, cos_mat), output_shape=(len_src,))(input)
        fft_layer_i = Lambda(lambda x: K.dot(x, sin_mat), output_shape=(len_src,))(input)
        fft_layer = concatenate([fft_layer_i, fft_layer_r], axis=-1)
        model = Dense(units=100, activation='relu')(fft_layer)
    ...
    def build_gan(self):
        ...
        valid_t = self.dis(signal)
        valid_f = self.dis_f(signal)
        valid_t = lambda x: x * sf_rate, output_shape=(1,))(valid_t)
        valid_f = lambda x: x * sf_rate, output_shape=(1,))(valid_f)
        output = lambda x: x[0] + x[1], output_shape=(1,))([valid_t, valid_f])
        # Add()([valid_f, valid_t])
    ...
    def train_dis(self, flag=None, flag_num=None):
        ...
        for i in range(num_batch):
            ...
            if flag == 'unroll' and flag_num == 0:
                with open('{0}/dis_param_unroll.hdf5'.format(self.filepath), 'w') as f:
    ...
    def train(self, epoch):
        ...
        with open('{0}/gene_param.hdf5'.format(self.filepath), 'w') as f:
            self.gene.save_weights(f.name)    
        with open('{0}/dis_param.hdf5'.format(self.filepath), 'w') as f:
            self.dis.save_weights(f.name)    
        with open('{0}/gan_param.hdf5'.format(self.filepath), 'w') as f:
            self.gan.save_weights(f.name)


    def normal_train(self):
        self.num_batch = int(np.ceil(num_train / batch_size))
    ...
    def unrolled_train(self):
        ...
        self.num_batch = int(np.ceil(self.y.shape[0] / batch_size))
        ...
        for i in range(self.num_batch):
            for roll in range(nroll):
                ...
                if roll == 0: self.dis.save_weights('{0}/dis_param_unroll.hdf5'.format(self.filepath))
            
            self.dis.load_weights('{0}/dis_param_unroll.hdf5'.format(self.filepath))
    ...
    def output_plot(self, num):
        ...
        for i in range(12):
            ...
            plt.ylim([0,1])
        plt.savefig('{0}/figure/epoch{1}.png'.format(self.filepath, num))
        plt.close()
...
class create_model():
    ...
    def build_freq_discriminator(self):
        ...
        # with tf.device('/cpu:0'):
        input = Input(shape = (None, feature_count + class_num))
        f_layer = Lambda(lambda x: x[:, 1:], output_shape=(feature_count,))(input)
        c_layer = Lambda(lambda x: x[:, 1:], output_shape=(class_num,))(input)
    ...
    def build_gan(self):
        ...
        self.dis.trainable = False
        self.dis_f.trainable = False
        valid_t = self.dis(input_comb_dis)
        valid_f = self.dis_f(input_comb_dis)
        valid_t = lambda x: x * sf_rate, output_shape=(1,))(valid_t)
        valid_f = lambda x: x * sf_rate, output_shape=(1,))(valid_f)
        output = lambda x: x[0] + x[1], output_shape=(1,))([valid_t, valid_f])
        # Add()([valid_f, valid_t])
        return Model(inputs=[z, class_info_gene, class_info_dis], outputs=output)
