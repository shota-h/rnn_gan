...
from keras import initializers as initial
...
if __name__=='__main__':
    ...
    with tf.Session() as sess:
        writer = tf.summary.FileWriter('rnn_gan_main_freeze',sess.graph)
        ...
        # GAN = form_gan(G,D)
        # v_y = np.append(np.zeros([v_x.shape[0],1,1]),np.ones([v_x.shape[0],1,1]),axis=0)
        loss_d_mat = []
        loss_g_mat = []
        predict_d_mat = []
        predict_g_mat = []
        ...
        for epoch in range(epoch_num):
            ...
            if (epoch+1)%100 == 0 and (b_num+1) == batch_num:
                ...
                summary = tf.Summary(value=[tf.Summary.Value(tag="loss_d_real",simple_value=loss_d_real),tf.Summary.Value(tag="loss_d_fake",,simple_value=loss_d_fake),tf.Summary.Value(tag="loss_gan",simple_value=loss_gan),tf.Summary.Value(tag="predict_x",simple_value=predict_d),tf.Summary.Value(tag="predict_z",simple_value=predict_g), ])
